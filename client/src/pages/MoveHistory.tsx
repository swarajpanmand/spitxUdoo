              ...
              <tbody>
                {filteredMoves.map((move) => (
                  <React.Fragment key={move.id}>
                    <tr className={`move-row ${move.products ? 'has-products' : ''}`}>
                      ...
                      <td>{move.quantity}</td>
                      <td>
                        <span className={`status-badge ${getStatusClass(move.status, move.type)}`}>
                          {move.status}
                        </span>
                      </td>
                    </tr>
                    {move.products && move.products.map((product, idx) => (
                      <tr key={`${move.id}-product-${idx}`} className="product-row">
                        <td colSpan={2}></td>
                        <td colSpan={3} className="product-name">↳ {product.name}</td>
                ...
                {filteredMoves
                  ...
                  .map((move) => (
                    <div key={move.id} className="kanban-card card">
                      ...
                      <div className="kanban-quantity">Qty: {move.quantity}</div>
                      <span className={`status-badge ${getStatusClass(move.status, move.type)}`}>
                        {move.type === 'in' ? 'IN' : 'OUT'}
                      </span>
                    </div>
